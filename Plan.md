...
Объект тестирования - веб-сервис: [Купить тур в Марракеш](http://localhost:8080/)
...
- Номер карты (международные стандарты) – от 13 до 19 цифр
...
Предусловие:
- Открыть веб-сервис по адресу: [Купить тур в Марракеш](http://localhost:8080/)
...
Валидные данные, успешная операция  
Описание (Summary)  
- В поле Номер карты ввести **случайный 13-тизначный номер**
- Остальные поля заполнить валидными данными
...
Валидные данные, успешная операция  
Описание (Summary)  
- В поле Номер карты ввести **случайный 19-тизначный номер**
- Остальные поля заполнить валидными данными
...
Валидные данные, успешная операция  
Описание (Summary)  
- В поле Год ввести **будущий год, но не превышающий +5 лет**
- Остальные поля заполнить валидными данными
...
Ожидаемый результат (Expected results)  
**В области незаполненного поля сообщение: Неверный формат   
Данные: не внесены в БД**

#### 1.3.2. Набор тестов с вводом латинских символов

**ID 12 - 15**  
Описание (Summary)  
- В целевое поле (Номер карты / Месяц / Год / CVC/CVV) ввести набор латинских символов: **setoflatincharacters**
- Остальные поля заполнить валидными данными

Ожидаемый результат (Expected results)  
**Целевое поле остается незаполненным  
В области поля с вводом латинских символов сообщение: Неверный формат   
Данные: не внесены в БД**
...
Ожидаемый результат (Expected results)  
**Целевое поле остается незаполненным  
В области поля с вводом кириллических символов сообщение: Неверный формат   
Данные: не внесены в БД**
...
Ожидаемый результат (Expected results)  
**Целевое поле остается незаполненным  
В области поля с вводом произвольных символов сообщение: Неверный формат   
Данные: не внесены в БД**
...
Ожидаемый результат (Expected results)  
**В области поля Месяц сообщение: Неверно указан срок действия карты  
Данные: не внесены в БД**
...
Ожидаемый результат (Expected results)  
**В области поля Год сообщение: Истёк срок действия карты  
Данные: не внесены в БД**

**ID 31**   
Название (Title)   
Проверка поля Владелец на ограничение символов  
Описание (Summary)
